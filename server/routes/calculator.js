...
import { Calculator, Paper, Price } from "../models/Calculator.js";
...
calculatorRoutes.post("/update-calculator", async (req, res) => {
  ...
  let { id,title, tag, products, pricing, papers, minMaxWidth, minMaxHeight } = req.body;
  let updates = {
    title: title,
    tag: tag,
    products: products,
  ...
  updates.paper == null ? delete updates.paper : "";
  try {
...
calculatorRoutes.get("/products", async (req, res) => {
  ...
  try {
    ...
    res
      .json({
        collectionData: data.body.collection,
      })
      .status(200);
  } catch (err) {
    res
...
calculatorRoutes.post("/create-paper", async (req, res) => {
  const { shop } = await clientProvider.graphqlClient({
    req,
    res,
    isOnline: true,
  ...
  pricingData.pricing = pricing;
  try {
    let priceData = new Paper(pricingData);
    let data = await priceData.save();
    res.json(data).status(200);
  } catch (err) {
    res
...
calculatorRoutes.delete("/delete-paper", async (req, res) => {
  try {
    const { ids } = req.body;

    const calculatorsToUpdate = await Calculator.find({ paper: { $in: ids } });

    for (const calculator of calculatorsToUpdate) {
      calculator.paper = null;
      await calculator.save();
    }
...
calculatorRoutes.get("/papers", async (req, res) => {
  const { shop } = await clientProvider.graphqlClient({
    req,
    res,
    isOnline: true,
  ...
  try {
    let data = await Paper.find({ store: shop });
    res.json(data).status(200);
  } catch (err) {
    res.json(err.message).status(400);
  }
...
calculatorRoutes.get("/papercount", async (req, res) => {
  const { shop } = await clientProvider.graphqlClient({
    req,
    res,
    isOnline: true,
  ...
  try {
    let data = await Paper.find({ store: shop }).countDocuments();
    res.json(data).status(200);
  } catch (err) {
    res.json(err.message).status(400);
  }
...
calculatorRoutes.get("/paper", async (req, res) => {
  let { id } = req.query;
  try {
    let data = await Paper.findById(id);
    res.json(data).status(200);
  } catch (err) {
    console.log(err.message);
    res.json(err.message).status(400);
...
calculatorRoutes.post("/update-paper", async (req, res) => {
  let { id, pricing } = req.body;
  pricing = pricing.map((el) => {
    el.id = new mongoose.mongo.ObjectId();
    return { ...el };
  ...
  try {
    let data = await Paper.findByIdAndUpdate(
      id,
      { pricing: pricing },
      { new: true }
    );
